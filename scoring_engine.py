"""
╔══════════════════════════════════════════════════════════════╗
║         HORIZONTES — ScoringEngine v1.1 (+ doctrina QMJ jun-2026) ║
...
Actualización jun-2026: umbrales Graham por norma contable (US_GAAP/IFRS/
AR_GAAP), proxy QMJ (Asness, Frazzini & Pedersen 2013 — DocC §2.2) y
QMJ Hard Override (DocC §4.2: proxy<40 → InvestmentScore capado en 65).
"""
...
class NormaContable(str, Enum):
    """Norma contable del emisor. Doctrina jun-2026: umbrales Graham
    uniformes entre jurisdicciones = bug P0."""
    US_GAAP = "US_GAAP"
    IFRS    = "IFRS"
    AR_GAAP = "AR_GAAP"


# Umbrales Graham por norma contable.
# · US_GAAP replica EXACTAMENTE los umbrales previos (retrocompatibilidad:
#   los 5 casos canarios históricos no cambian).
# · ⚠️ IFRS y AR_GAAP: valores con racional documentado — revisar contra
#   la contabilidad real de tus emisores IFRS/AR_GAAP antes de confiar
#   el filtro hard en producción.
#   IFRS: IFRS-16 capitaliza leases → infla pasivo corriente → CR exigible menor.
#   AR_GAAP: ajuste por inflación (RT 6/RT 48) distorsiona capital de trabajo y
#   resultados → CR menor, P/E más exigente, margen de seguridad mayor.
GRAHAM_THRESHOLDS: dict[NormaContable, dict[str, float]] = {
    NormaContable.US_GAAP: dict(cr_min=2.0, de_ok=0.5, de_max=1.0,
                                ic_ok=4.3, ic_min=2.0,
                                pe_ok=15.0, pe_max=25.0,
                                ms_ok=0.30, ms_min=0.15),
    NormaContable.IFRS:    dict(cr_min=1.8, de_ok=0.6, de_max=1.2,
                                ic_ok=4.0, ic_min=2.0,
                                pe_ok=15.0, pe_max=25.0,
                                ms_ok=0.30, ms_min=0.15),
    NormaContable.AR_GAAP: dict(cr_min=1.5, de_ok=0.6, de_max=1.2,
                                ic_ok=3.5, ic_min=1.8,
                                pe_ok=10.0, pe_max=18.0,
                                ms_ok=0.35, ms_min=0.20),
}

# Doctrina QMJ (DocC §4.2, textual): "Si el QMJ Score de un activo es inferior
# a 40/100 (clasificación 'Junk'), el InvestmentScore total no puede superar
# 65/100 independientemente del PEG de Lynch o del P/E de Graham."
QMJ_JUNK_THRESHOLD = 40.0
QMJ_SCORE_CAP      = 65.0

# Tabla pesos validada (suma exacta = 1.0 por perfil)
...
HOUSEL_NARRATIVES: dict[str, dict] = {
    ...
    "qmj_junk": {
        "titulo":    "🧱 Trampa de Valor — Calidad Insuficiente (Override QMJ)",
        "mensaje": (
            "Los múltiplos lucen baratos, pero el negocio detrás es de baja "
            "calidad: rentabilidad pobre, balance frágil o caja que no acompaña "
            "las ganancias. Lo barato que es 'junk' suele estar barato por algo. "
            "Horizontes capó este score: pagar poco por un mal negocio no es "
            "margen de seguridad — es la tarifa de entrada a un problema."
        ),
    },
    "qmj_watch": {
        "titulo":    "🟠 Score Alto, Calidad Justa — Vigilar el QMJ",
        "mensaje": (
            "El score compuesto es atractivo, pero el proxy de calidad quedó "
            "apenas por encima de la zona 'junk'. La historia premia a los "
            "negocios de calidad sostenida, no a los múltiplos ocasionalmente "
            "baratos. Si entrás, hacelo con posición moderada y revisá que la "
            "rentabilidad y el balance mejoren, no solo el precio."
        ),
    },
}
...
class MetricasFinancieras(BaseModel):
    ...
    # ── Norma contable (doctrina jun-2026: umbrales Graham diferenciados) ─
    norma_contable:    NormaContable = Field(
        NormaContable.US_GAAP,
        description="Norma contable del emisor — selecciona la tabla de umbrales Graham")
...
def score_graham(m: MetricasFinancieras, perfil: PerfilInversor) -> tuple[float, dict]:
    ...
    Umbrales por norma contable (GRAHAM_THRESHOLDS) — doctrina jun-2026.
    Filtro hard: current_ratio < cr_min → penalización severa.
    """
    puntos = 0.0
    detalle: dict = {}
    t = GRAHAM_THRESHOLDS[m.norma_contable]
    detalle["norma"] = f"Umbrales {m.norma_contable.value}"

    # --- Filtro Hard: Current Ratio ---
    if m.current_ratio is not None:
        if m.current_ratio >= t["cr_min"]:
            puntos += 20
            detalle["current_ratio"] = f"✅ {m.current_ratio:.2f} ≥ {t['cr_min']:.1f} (+20pts)"
        else:
            puntos -= 30   # Hard reject Graham
            detalle["current_ratio"] = (f"🚫 {m.current_ratio:.2f} < {t['cr_min']:.1f} "
                                        f"(Rechazo automático -30pts)")

    # --- Deuda/Equity ---
    if m.debt_equity is not None:
        if m.debt_equity <= t["de_ok"]:
            puntos += 20
            detalle["debt_equity"] = f"✅ {m.debt_equity:.2f} ≤ {t['de_ok']:.1f} (+20pts)"
        elif m.debt_equity <= t["de_max"]:
            puntos += 10
            detalle["debt_equity"] = f"⚠️ {m.debt_equity:.2f} moderado (+10pts)"
        else:
            puntos -= 10
            detalle["debt_equity"] = f"❌ {m.debt_equity:.2f} > {t['de_max']:.1f} (-10pts)"

    # --- Cobertura de intereses (benchmark Graham por norma) ---
    if m.interest_coverage is not None:
        if m.interest_coverage >= t["ic_ok"]:
            puntos += 15
            detalle["interest_coverage"] = f"✅ {m.interest_coverage:.1f}x ≥ {t['ic_ok']:.1f}x (+15pts)"
        elif m.interest_coverage >= t["ic_min"]:
            puntos += 5
            detalle["interest_coverage"] = f"⚠️ {m.interest_coverage:.1f}x (+5pts)"
        else:
            puntos -= 15
            detalle["interest_coverage"] = f"❌ {m.interest_coverage:.1f}x < {t['ic_min']:.1f}x (-15pts)"

    # --- P/E ratio ---
    if m.pe_ratio is not None:
        if m.pe_ratio <= t["pe_ok"]:
            puntos += 20
            detalle["pe_ratio"] = f"✅ P/E {m.pe_ratio:.1f} ≤ {t['pe_ok']:.0f} (+20pts)"
        elif m.pe_ratio <= t["pe_max"]:
            puntos += 8
            detalle["pe_ratio"] = f"⚠️ P/E {m.pe_ratio:.1f} aceptable (+8pts)"
        else:
            puntos -= 5
            detalle["pe_ratio"] = f"❌ P/E {m.pe_ratio:.1f} > {t['pe_max']:.0f} (-5pts)"

    # --- Margen de Seguridad ---
    if m.margen_seguridad is not None:
        if m.margen_seguridad >= t["ms_ok"]:
            puntos += 25
            detalle["margen_seguridad"] = f"✅ {m.margen_seguridad*100:.0f}% ≥ {t['ms_ok']*100:.0f}% (+25pts)"
        elif m.margen_seguridad >= t["ms_min"]:
            puntos += 12
            detalle["margen_seguridad"] = f"⚠️ {m.margen_seguridad*100:.0f}% (+12pts)"
        else:
            puntos += 0
            detalle["margen_seguridad"] = f"❌ {m.margen_seguridad*100:.0f}% < {t['ms_min']*100:.0f}% (0pts)"
...
# ─────────────────────────────────────────────────────────────────────────────
# 3b. PROXY QMJ — QUALITY MINUS JUNK (DocC §2.2, Asness/Frazzini/Pedersen 2013)
# ─────────────────────────────────────────────────────────────────────────────

def calc_qmj_proxy(m: MetricasFinancieras) -> tuple[Optional[float], dict]:
    """
    Proxy del Quality Score QMJ con las métricas disponibles del modelo.
    Doctrina DocC: Quality = (Profitability + Growth + Safety + Payout) / 4.
    · Payout NO es computable (el modelo no trae payout ratio/buybacks) —
      se promedian las dimensiones disponibles y se documenta.
    · Devuelve (proxy 0-100, detalle). None si hay < 2 dimensiones con datos:
      sin evidencia suficiente NO se clasifica 'junk' (evita falsos override).
    """
    dims: dict[str, float] = {}
    det:  dict = {}

    # — Profitability: ROE (15% quality / 30% pleno), ROIC (12% media / 24% pleno),
    #   flujo_caja_score ya viene 0-100 con 50 ≙ FCF yield 4% (umbral DocC).
    prof: list[float] = []
    if m.roe is not None:
        prof.append(_clamp(m.roe / 30 * 100))
    if m.roic is not None:
        prof.append(_clamp(m.roic / 24 * 100))
    if m.flujo_caja_score is not None:
        prof.append(_clamp(m.flujo_caja_score))
    if prof:
        dims["profitability"] = sum(prof) / len(prof)

    # — Growth (calidad del crecimiento, no magnitud): banda sostenible 5–25%
    #   premia; negativo penaliza; >25% decae (sospecha de crecimiento
    #   apalancado/dilutivo por encima de g_sostenible — DocC §2.2 Dim.2).
    if m.crecimiento_bpa is not None:
        g = m.crecimiento_bpa
        if g < 0:
            dims["growth"] = _clamp(50 + g * 2.5)          # −20% → 0
        elif g <= 25:
            dims["growth"] = _clamp(50 + g * 2)            # +25% → 100
        elif g <= 40:
            dims["growth"] = _clamp(100 - (g - 25) * 2)    # decae hacia 70
        else:
            dims["growth"] = 60.0

    # — Safety: D/E (<0.5 alta), cobertura de intereses (10x plena),
    #   current ratio (proxy de liquidez).
    saf: list[float] = []
    if m.debt_equity is not None:
        saf.append(_clamp(100 - m.debt_equity * 50))       # 0→100 · 1→50 · 2→0
    if m.interest_coverage is not None:
        saf.append(_clamp((m.interest_coverage - 1) / 9 * 100))
    if m.current_ratio is not None:
        saf.append(_clamp((m.current_ratio - 1) / 2 * 100))
    if saf:
        dims["safety"] = sum(saf) / len(saf)

    if len(dims) < 2:
        return None, {"qmj": "Datos insuficientes para proxy QMJ (se omite override)"}

    proxy = sum(dims.values()) / len(dims)
    for k, v in dims.items():
        det[f"qmj_{k}"] = round(v, 1)
    det["qmj_payout"] = "n/d (sin payout/buybacks en el modelo — DocC §2.2)"
    det["qmj_proxy"] = round(proxy, 1)
    return round(proxy, 1), det
...
class ScoringEngine:
    ...
    def calcular(self, req: ScoringRequest) -> InvestmentScore:
        ...
        # ── Doctrina QMJ Hard Override (DocC §4.2, jun-2026) ──────────────
        qmj_proxy, qmj_det = calc_qmj_proxy(m)
        bloques["investingpro"].detalle.update(qmj_det)
        qmj_override = False
        if qmj_proxy is not None and qmj_proxy < QMJ_JUNK_THRESHOLD:
            if score_final > QMJ_SCORE_CAP:
                score_final = QMJ_SCORE_CAP
                qmj_override = True
                alertas.append(
                    f"🧱 QMJ HARD OVERRIDE: proxy de calidad {qmj_proxy:.0f}/100 < "
                    f"{QMJ_JUNK_THRESHOLD:.0f} ('Junk'). Score capado a "
                    f"{QMJ_SCORE_CAP:.0f}/100 — un negocio de baja calidad cotizando "
                    f"barato suele ser trampa de valor, no oportunidad (Asness et al., 2013)."
                )
            else:
                alertas.append(
                    f"🧱 Calidad 'Junk' (QMJ proxy {qmj_proxy:.0f}/100 < "
                    f"{QMJ_JUNK_THRESHOLD:.0f}): el score ya refleja la debilidad."
                )

        # ── Doctrina cripto-beta (jun-2026) ───────────────────────────────
        if req.ticker.upper().replace(".BA", "") == "MSTR":
            alertas.append(
                "₿ MSTR: exposición cripto-beta con amplificador 1.5× (tope factor 1.0). "
                "Doctrina jun-2026: PROHIBIDO clasificar MSTR como cobertura (hedge)."
            )
        ...
        # Narrativas Housel específicas de escenarios QMJ (jun-2026)
        if qmj_override:
            nivel_narrativa = "qmj_junk"
        elif (qmj_proxy is not None
              and QMJ_JUNK_THRESHOLD <= qmj_proxy < 50
              and clasificacion in ("STRONG BUY", "ACCUMULATE")):
            nivel_narrativa = "qmj_watch"
        ...
        return InvestmentScore(
            ...
            alertas=alertas,
            qmj_proxy=qmj_proxy,
            qmj_override_aplicado=qmj_override,
        )
